...
const PenawaranPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await http(user).post("/penawaran", {
        serialnumber: serialNumber,
        tipe,
        part,
      });
  ...
  const handleDelete = async (id) => {
    try {
      const response = await http(user).delete(`/penawaran/${id}`);
      fetchData();
    } catch (error) {
      console.error(error);
    }
  ...
  const fetchData = async () => {
    try {
      const response = await http(user).get("/penawaran");
...
export default WithAuthProtection(PenawaranPage);
